...
//      List available backup files
	...
	foreach ($folder as $backname) {
		if (eregi("__",$backname)) {    //show only folder with two _ in its name 
            ...
            if ($fcount > '2') {        //show only folder that are not empty              
    			$count_backup++ ;
    			echo "<dt class='$bgcolor x2' style='padding:9px;'>$backname</dt>
        			<dd class='$bgcolor cntr'>
        			<input class='sbmt' type='button' name='lrestore'
        			onclick=\"confirm_rest_prompt('./admin.php?f=database&amp;file=$backname&amp;del=0');\" value='Restore' 
        			...
        			onclick=\"confirm_del_prompt('./admin.php?f=database&amp;file=$backname&amp;del=1');\" value='Delete'
	...
	echo "</dl>
		<div class='panel x2 cntr'>
			<p class='evrow'>Create a new Backup File from current database<br />
			<input type='hidden' name='f' value='database'
			/>
			<input type='checkbox' name='structonly' value='Yes'
			/> Backup structure only<br />
			<input class='sbmt' type='submit' name='send2' value='Backup'
			title='Beware! Once started, the database backup could take some while to complete!'
			/></p>
		</div>
		</form>
	</div>
...
//      Enter here to backup current db
	if($send2 == "Backup") {
		...
		optimize($database);//      before backup, preventively repair and optimize current database    
		...
		if (eregi("__",$file)) {
			...
			if ($dh = opendir($dir)) {  
					...
					if (eregi("\.gz$", $dbfile)) {
						...
						while(!gzeof($zp)){
							...
							if ($endoff = strpos($temp, $delimiter)) {  //find end of sql insert 
								...
								mysql_db_query($database,$query) or die(mysql_error()); //insert into table 
								$temp = '';
							}
						}
						gzclose($zp); 
				...
				echo " <p class='odrow cntr'>
                 ...
                 <p class='evrow cntr'>
                 <a class='bkbtn' href='admin.php?f=database' title='Go back to Database'>Complete this process</a></p>                 
                ";
