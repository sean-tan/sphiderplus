    ...
    if ($num_rows > '1') {  //      Multiple choice
        ...
        for ($i=0; $i<$num_rows; $i++) {
            ...
            echo "
                ...
                <td>
                <a href='./search.php?query=site:$url2&search=1'> $url2 </a>
                </td>
                <td>                    
                indexed: $indexdate
            ";
    ...
    if ($num_rows > 0) { 
        ...
        if ($pages > 1) { // If we have more than 1 result-page 
            ...
            if($start_links > 1) { // Display 'First'            
                echo "
                    <a href='search.php?query=$query&start_links=1&search=1'>".$sph_messages['First']."</a>&nbsp;&nbsp;
                ";
                
                if ($start_links > 5 ) { // Display '-5' 
                $minus = $start_links-5;
                echo " 
                    <a href='search.php?query=$query&start_links=$minus&search=1'>- 5</a>&nbsp;&nbsp; 
                "; 
                } 
            } 
            if($start_links > 1) { // Display 'Previous' 
                $prev = $start_links-1;
                echo " 
                    <a href='search.php?query=$query&start_links=$prev&search=1'>".$sph_messages['Previous']."</a>&nbsp;&nbsp; 
                ";
            } 
            if($rest >= $results_per_page) { // Display 'Next'
                $inc = $start_links+1;               
                echo " 
                    <a href='search.php?query=$query&start_links=$inc&search=1' >".$sph_messages['Next']."</a>&nbsp;&nbsp; 
                ";
                
                if ($pages-$start_links > 5 ) { // Display '+5' 
                    $plus = $start_links+5;
                    echo " 
                     <a href='search.php?query=$query&start_links=$plus&search=1'>+ 5</a>&nbsp;&nbsp; 
                    "; 
                } 
            } 
            if($start_links < $pages) { // Display 'Last' 
            echo " 
                 <a href='search.php?query=$query&start_links=$pages&search=1'>".$sph_messages['Last']."</a> 
                "; 
            }
            echo "</div>
            ";
